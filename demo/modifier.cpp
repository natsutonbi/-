...
State_modifier::State_modifier()//默认全设0
{
    for(int i=0;i<20;++i)
    {
        data[i]=0;
...
State_modifier::State_modifier(const double source[20])//copy
{
    for(int i=0;i<20;++i)
    {
        data[i]=source[i];
...
const State_modifier& State_modifier::operator +(const State_modifier& add) const
{
    double temp[20];
    for(int i=0;i<20;++i)
    {
        temp[i]=data[i]+add.data[i];
    }
    return State_modifier(temp);
}
const State_modifier& State_modifier::operator +(const double add[20]) const
{
    double temp[20];
    for(int i=0;i<20;++i)
    {
