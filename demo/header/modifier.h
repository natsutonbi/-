...
public:
    ...
    State_modifier();//默认全0
    State_modifier(const double source[20]);
    const State_modifier& operator +(const State_modifier& add) const;//属性相加
    const State_modifier& operator +(const double add[20]) const;
    const State_modifier& operator -(const State_modifier& add) const;//属性相加
    const State_modifier& operator -(const double add[20]) const;
